...
double AverageAboveDig(int num, int digit, int *counter)
{
    ...
    {
        *counter += 1;
        return result = (AverageAboveDig(num / 10, digit, counter) + (double)(num % 10)) / (double)2;
    }
    else return result = AverageAboveDig(num / 10, digit, counter);
}